...
# Test the CORRECT format you provided
...
payload = {
    ...
    "pipeline_inputs": [
        {
            "input_name": "link",
            "value": "https://www.youtube.com/@FreyChu"
        }
    ]
}
headers = {
...
try:
    ...
    if 'run_id' in data:
        ...
        print(f"\n🔗 Correct format test run_id: {new_run_id}")
        
        # Wait and then check result
        import time
        print("Waiting 2 minutes for processing...")
        time.sleep(120)
        ...
        if outputs and 'output' in outputs:
            ...
            if 'FreyChu' in output_content or 'Frey' in output_content:
                print("✅ SUCCESS: Correct format got FreyChu content!")
                print(f"Preview: {output_content[:300]}...")
            elif 'Tjas' in output_content:
                print("❌ STILL WRONG: Correct format still got Tjas content!")
